...
\centering
        \resizebox{!}{0.7\paperheight}{%
            ...
            child[concept color=hriSec2,text=black, grow=120,visible on=<5->] { node[concept] (action) {Performing with humans} 
                ...
                \uncover<6->{
                    \node[above right=0.1 of attention, pin=above right:Katrin L.] {};
                    \node[right=0.1 of attention, pin=right:Frank B.] {};
                    \node[pin=50:Oliver L.] at ($(comm) + (50:1)$)  {};
                    \node[above left=0.1 of long-term, pin=above left:Christian D.] {};
                    \node[left=0.1 of dialog, pin=left:Verena R.] {};
                    \node[below left=0.1 of dialog, pin=below left:Jekaterina N.] {};
                    \node[pin=260:Simon K.] at ($(dialog) + (260:1.2)$)  {};
                    \node[above left=0.1 of dialog, pin=above left:Helen H.] {};
                    \node[below=0.1 of memory, pin=below:Helen H.] {};
                    \node[above right=0.1 of memory, pin=above right:Patricia V.] {};
                    \node[above=0.1 of emotions, pin=above:Jekaterina N.] {};
                    \node[pin=260:Ruth A.] at ($(emotions) + (245:0.8)$)  {};
                    \node[above=0.1 of knowledge, pin=above:Ron P.] {};
                    \node[above left=0.1 of joint-action, pin=above left:Ron P.] {};
...
\begin{frame}{Online Instantiation}
    ...
    \begin{columns}
        ...
        \centering
        \includegraphics[width=\linewidth]{pr2-scene.jpg}
...
\againframe<2>{symbolicrelations}
...
%    \end{multicols}
    ...
    \centering
        \resizebox{!}{0.6\paperheight}{%
            ...
            child[concept color=hriSec2Comp,text=white,grow=-45] { node[concept] (knowledge) {Social Knowledge} 
                [counterclockwise from=-140]
                child[concept color=hriSec1CompDark,text=white] { node[concept] (soc-rules) {Social rules} }
                child[concept color=hriSec3Comp,text=black] { node[concept] (soc-ctxt) {Social context} }
                child[concept color=hriSec2Dark,text=white] { node[concept] (memory) {Social memory} }
            ...
            child[concept color=hriSec2,text=black, grow=120] { node[concept] (action) {Performing with humans} 
                ...
                \node[circle, draw=red, very thick, minimum size=2cm] at (memory.center) {};
                ...
                \node[circle, draw=red, very thick, minimum size=1.6cm] at (non-verbal.center) {};
...
\paper{Lemaignan et al. {\bf Artificial Cognition for Social Human-Robot
     Interaction: An Implementation} Artifical Intelligence, 2016}
\begin{frame}{Human-robot joint task achievement}

    \begin{center}

        Walkthrough of one full-stack example: for \hyperlink{cleanthetable}{the questions}
     \vspace{2em}
...
\begin{frame}[plain]
    ...
    \Large\centering
    \textbf{Since my PhD, a lot of experimental work}

    \pause
    One example that shows that 'socio-cognitive robotics' is not just a
    buzz: the \emph{CoWriter} project
\end{frame}
}
...
\begin{frame}{Scaffolding the research}
    ...
    What are the next challenges for artificial social cognition?
    ...
    \begin{itemize}
        ...
        %\item<+-> Challenge to isolate behaviours, analyses them (and if we are
...
\begin{frame}{What social behaviours do I aim for?}
...
%            awareness 
        ...
        \item {\bf replication of Parten's hierarchy} $\leftarrow$ ambitious!


    \end{itemize}

    $\Rightarrow$ a research programme for the next 5-10 years
\end{frame}

\begin{frame}{My research plan}
    ...
    \begin{tikzpicture}[
        ...
        \node (top) {\large Two tracks};

        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        \node[below left=of top] (practical) {\emph{Practical}} edge (top);
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        \node<1>[below=0.2 of practical,text width=5.5cm] (practicaldesc) {
            \begin{itemize}
                \item initially symbolic approach
                \item extended representation capabilities (like
                pre-supposition accomodation)
                \item multi-platform
                \item support integration at HW HRI group level
            \end{itemize} };
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        \node<2>[below=0.2 of practical,text width=5.5cm] {
            ...
            The ``Edinburgh Architecture for Socio-Cognitive Robots''
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        \node<1>[below=0.2 of basic, text width=5cm] (basicdesc) {
            \begin{itemize}
                \item principled approach
                \item emergent/developmental paradigm
                \item inspiration from neurosciences (\emph{\hyperlink{attentionschemata}{attention schemata
                    theory}})
        ...
        \node<2>[below=0.2 of basic, text width=5cm] {
            ...
            \includegraphics[width=\linewidth]{memory/weights-zoo}

            Emergent Artificial Social Cognition

        };

        \coordinate (bottom) at (top |- practicaldesc.south);
        \node<1>[below=0.2 of bottom] (hyb) {cross-pollination};
...
\begin{frame}{A framework for free play}

    \begin{center}
        \only<1>{
            \includegraphics[width=0.8\linewidth]{next/zoo-activity2}
        }
        \only<2>{
            \video{\linewidth}{figs/next/zoo-builder-proto-smaller.mkv}
        }
        \only<3>{
            \includegraphics[width=\linewidth]{memory/zoo+ros+memory}
        }
        \only<4>{
            \includegraphics[width=0.8\linewidth]{memory/weights-zoo}
        }
    \end{center}
\end{frame}
...
\begin{frame}{Developing Child-Robot Interaction at Heriot Watt}
    ...
    \begin{columns}
        \begin{column}{0.4\linewidth}
            \begin{center}
                ...
                \uncover<2>{
                \includegraphics[width=\linewidth]{edinburgh/sickchildrenhospital}
                }
            \end{center}
        \end{column}
        \begin{column}{0.6\linewidth}
            ...
            Create a \textbf{KidsLab}, within the Robotarium
                \begin{itemize}
                    ...
                    \item in connection with UoE Wee Sciences lab and the RabLab
                \end{itemize}

            \vspace{2em}

            \uncover<2>{
            Establish collaboration with the (new) Royal Hospital for Sick
            Children
            \begin{itemize}
                \item field experiments
                \item research with real-world impact on children
            \end{itemize}
        }
        \end{column}
    \end{columns}
...
\begin{frame}{Funding}
    ...
    \begin{itemize}
        \item {\bf EPSRC First Grant}
            \begin{itemize}
                ...
                \item max 2 years, no closing date to submit
                \item covers equipment + 1 post-doc
                \item fund the \textbf{KidsLab} project
            \end{itemize}
        \item {\bf EPSRC Early Career Fellowship}
            \begin{itemize}
                \item max 5 years, no closing date to submit
                \item covers equipment and staff
    ...
    \begin{itemize}
        \item {\bf Co-Investigator on a H2020 project}
            \begin{itemize}
                \item close contacts with IST Lisbon, LAAS-CNRS, Plymouth, EPFL,
                    INRIA Bordeaux
            \end{itemize}
        \item Submit a {\bf EPSRC Standard Research} proposal
            \begin{itemize}
                \item aimed at reaching independence (1 PhD, 1 post-doc)
                \item focus on practical, long-term field deployments (\eg Sick
                    Children hospital)
...
\begin{frame}{What else?}
        ...
        \begin{tikzpicture}
            \node[rotate=90,text width=2cm]{\textbf{Social HRI\\on the field}};
        \end{tikzpicture}
...

{
    \paper{Lemaignan et al., {\bf Artificial Cognition for Social Human-Robot
    Interaction: An Implementation}, Artifical Intelligence, 2016}
\begin{frame}[label=cleanthetable]{``Cleaning the table''...}
\centering
