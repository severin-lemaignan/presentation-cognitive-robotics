...
\begin{frame}{Academic Curriculum overview}
    ...
    \onslide<3->
     2013: post-doc at \textbf{EPFL}, establishing there a new leading group in
...
\begin{frame}{Teaching experience}
    ...
    \only<2> {
        ...
        \begin{itemize}
            ...
            \item advanced C++ (C++11, STL, meta-programming)
            \item advanced Python (meta-programming)
            \item logic programming (Prolog, ontologies)
            \item distributed systems (middlewares)
            \item software engineering (GIT expertise, coding best practises)
        ...
        \emph{120+ open-source repositories on GitHub; contributor to major
...
\begin{frame}[plain]
    ...
    \only<2>{
    \vspace{11em}
    \begin{exampleblock}{Complex challenges}

        \begin{itemize}
            \item Mix of geometry, time, semantics
            \item High geometric/temporal granularity of social signals (e.g. nodding)
            \item Common-sense knowledge, context, cultural background
                %\item Non-monotonic topology
                %\item Represent continuous/discrete fields
            \item Account for uncertainties
                %\item Imagine entities
            \item Social memory; ``remember the past to predict the future''
...
\begin{frame}[plain]
    ...
    \centering
    {\bf Situated dialogue} effectively evidences these challenges
...
\begin{frame}{Symbolic situation assessment}
...
\begin{frame}[plain]
    ...
    \Large\centering
    \textbf{Since my PhD, further experimental work to characterise human-robot
    interaction}
    ...
    \normalsize
    \vspace{2em}
    One example to evidence the importance of a \textbf{socio-cognitive
    approach} to HRI: the \emph{CoWriter} project
\end{frame}
}
    ...
    \begin{itemize}
        \item<+-> {\bf The robot as a cognitive agent is key here}
            \begin{itemize}
                ...
                \item metacognition
            \end{itemize}
        \item<+-> \textbf{New role}: not a 'tool to teach robotics', not a
            facilitator \\
        (note: a tool for the teacher vs a social agent for the child!)
...
\imageframe[caption=A framework for artificial socio-cognition]{memory/setup}
...
%            awareness 
    ...
    $\Rightarrow$ significant scientific targets for human-robot interaction
\end{frame}
...
\begin{frame}{Developing Child-Robot Interaction at Plymouth}

    \begin{columns}
        \begin{column}{0.4\linewidth}
            \begin{center}
                \includegraphics[width=\linewidth]{plymouth/babylab}
                \vspace{2em}
    ...
    \begin{itemize}
        \item {\bf EPSRC First Grant}
            \begin{itemize}
                ...
                \item covers equipment + one post-doc
            \end{itemize}
