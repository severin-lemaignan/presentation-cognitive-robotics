...
\date{University of Edinburgh -- {\bf 15 May 2017}}
...
%    \only<2> {
        ...
        open-source projects} like \textbf{ROS}, \textbf{OpenCV}
...
\begin{frame}<3>[label=sitass]{Multi-modal Symbolic situation assessment}
    ...
    \begin{columns}
        ...
        \node [instance, right=of human, anchor=north west] (book) {book\_harry\_potter}
        ...
        \node [instance, below=5 of act] (moving) {move\_to\_socket} edge[classof] (act);
        \path (moving.south west) edge [property, out=200, in=-80,looseness=0.9] node[propname,below] {currentlyPerforms} (human2);

        \path (robot.south) edge [property, out=-80, in=-90, looseness=2] node[propname,right] {looksAt} (human.290);
        \path (book.south) edge [property, out=-100, in=-90, looseness=1] node[propname,below] {looksAt} (human.270);
...
%        $\rightarrow$ {\tt book\_harry\_potter} \\
%    }
%
%    \uncover<3-> {
%        $\downarrow$\\
...
\begin{frame}{Parallel Models: towards a theory of mind}
        \begin{multicols}{2}
            \begin{figure}
                ...
                \node [instance, below=3 of agent] (human) {human\_1} edge[classof, bend left] (agent);
                \node [instance, above left=of human] (human2) {human\_2} edge[classof, bend left] (agent);
                \node [instance, above right=of human, anchor=south] (robot) {robot} edge[classof, bend left] (agent);
;
                \node [instance, right=of human, anchor=north west] (book) {book\_harry\_potter}
                edge[classof] (artifact);
                ...
                \path (book.200) edge [property, out=-100, in=-80, looseness=2] node[propname,auto] {looksAt} (human.south);
                \node [instance, below=5 of act] (moving) {move\_to\_socket} edge[classof] (act);
                \path (moving.south west) edge [property, out=200, in=-80,looseness=1] node[propname,below] {currentlyPerforms} (human2);
...
;
                ...
                node[propname,auto] {looksAt} (human.south);
                \path (book.270) edge [property, out=-100, in=-90, looseness=3.5] node[propname,auto] {looksAt} (robot.south);
                \path (ikea.south) edge [property, out=-90, in=-80, looseness=3] node[propname, auto] {isOn} (book.320);
                \end{tikzpicture}
            }
...
\begin{frame}{Other experimental work}
        ...
        \begin{tikzpicture}
            \node[rotate=90,text width=2.5cm]{\small\textbf{Understanding\\social\\interaction}};
        \end{tikzpicture}
...
\imageframe{figs/freeplay/freeplay-sandbox-architecture.pdf}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\videoframe[0.56]{videos/falsebeliefs.webm}
...

{
    \paper{Lemaignan et al., {\bf Artificial Cognition for Social Human-Robot
    Interaction: An Implementation}, Artifical Intelligence, 2016}
\begin{frame}<1-7>{Into an control architecture}
    \begin{figure}
        \centering
